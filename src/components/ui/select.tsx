...
>(({ className, children, invalid, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      "flex h-11 w-full items-center justify-between gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] px-3.5 text-sm shadow-inset outline-none transition-all duration-200 ease-premium",
      "data-[placeholder]:text-muted-foreground/70",
      "hover:border-white/[0.14] hover:bg-white/[0.045]",
      "focus:border-primary/60 focus:ring-4 focus:ring-primary/12",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      position={position}
      className={cn(
        "glass-strong relative z-50 max-h-[22rem] min-w-[8rem] overflow-hidden rounded-2xl p-1.5 text-popover-foreground shadow-lifted",
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center rounded-xl py-2 pl-3 pr-9 text-[13px] outline-none transition-colors",
